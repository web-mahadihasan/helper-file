...
const ProductsModalTest4 = ({
  ...
  // Reset all state on open (reset scroll ONLY here)
  useEffect(() => {
    if (isOpen) {
  ...
  }, [isOpen, selectedProducts, refetchProducts]);
  ...
  useEffect(() => {
    ...
    if (!isPaginating && (endCursor === null || endCursor === undefined)) {
  ...
  }, [modalProducts, isPaginating, endCursor]);

  // Scroll handler for infinite scroll
  const handleScroll = (e) => {
    if (productLoading || productFetching || isPaginating || !hasNextPage)
      return;
  ...
  // Filter/search handlers (reset scroll ONLY here)
  ...
  // Selection helpers (DO NOT reset scroll here)
    ...
    >
      ...
      >
        ...
        >
          <Image
            src={
              product.node?.featuredMedia?.preview?.image?.transformedSrc ||
              "/placeholder.svg"
            }
        ...
        >
          <Flex justify="space-between" align="center">
            <Text fontSize="md" fontWeight="semibold">
              Add products ({selection.length}/{maxProductSelect || 1}) test 4
            </Text>
...
export default ProductsModalTest4;
